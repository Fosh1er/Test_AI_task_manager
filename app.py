...
def load_transcript_from_yandex_disk(filename: str) -> str:
    """Загрузка файла транскрипции с Яндекс Диска."""
    ...
    if token:
        ...
        logger.info(f"Запрос к Яндекс Диску (Token): {full_path}")
        response = requests.get(url, headers=headers, params=params)
    elif public_key:
        # Доступ по публичной ссылке
        url = "https://cloud-api.yandex.net/v1/disk/public/resources/download"
        params = {"public_key": public_key, "path": filename}
        logger.info(f"Запрос к Яндекс Диску (Public Key): {filename}")
        response = requests.get(url, params=params)
    else:
        raise ValueError("Не задан YANDEX_DISK_TOKEN или YANDEX_DISK_PUBLIC_KEY в переменных окружения")
...
def load_transcript(filename: str) -> str:
    # Читаем источник из env (по умолчанию local)
    source = os.environ.get("TRANSCRIPT_SOURCE", "local").lower()

    if source == "yandex_disk":
        logger.info(f"Источник: Яндекс Диск. Файл: {filename}")
        return load_transcript_from_yandex_disk(filename)
    else:
        logger.info(f"Источник: Локальная папка. Файл: {filename}")
